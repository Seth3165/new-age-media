...
const PostView = ({id, date, profileImageUrl, title, description, file, username, currentUser, addFavorite, addArtist, removePost, isCorrectUser}) => (
  <div className="viewArea">
    <h2 className="viewTitle">{title}</h2>
    <div className="viewControl">
      <p className="viewInfo">{description}</p>
      <p>Posted by: <Link to={`/users/`} className="posterLink">{username}</Link></p>
